...
/* ── invented experience ────────────────────────────────────────────────────
   The Make writer was told to "write as him", and it did: "I have watched
   couples…", twice in each of two published notes, under a CEA registration.
   None of it happened. A piece may address the reader as "you"; it may not
   claim that anybody — Shervin, a client, a buyer — did, saw or said anything
   the sources do not record. Caught here so the publish button refuses it,
   and at intake so a draft carrying it is never filed. A note Shervin writes
   himself is a file in content/insights and never passes through either. */
export const EXPERIENCE = /\b(?:(?:clients?|buyers?|sellers?|owners?|agents?) (?:have )?(?:told|asked|said to|often tell)|(?:have|has) watched|in my experience|over the years,? I|a (?:client|reader|buyer) (?:once|recently) (?:told|asked))\b/i;
const FIRST_PERSON_EXPERIENCE = /\b(?:I (?:have|had|was|am|think|see|saw|watched|spoke|remember|tell|know|say|met|sat|walked|asked|find|would|often)|I've|I'd|[Mm]y (?:clients?|buyers?|sellers?|experience|years)|[Ww]e (?:have|see|saw|spoke|met)|[Ww]e've)\b/;

/** The invented-experience phrase in a piece, or null. */
export function inventedVoice(html) {
  const text = plain(html);
  const hit = FIRST_PERSON_EXPERIENCE.exec(text) || EXPERIENCE.exec(text);
...
export function publishBlockers(article) {
  ...
  const voice = inventedVoice(`${article?.title || ''} ${article?.excerpt || ''} ${article?.content_html || article?.body_html || ''}`);
  if (voice) {
    blockers.push({ id: 'invented-voice', why: 'It claims an experience nobody on record had, in the first '
      + `person or through an unnamed client, under a CEA registration. Found: “${voice}”.` });
  }
  return blockers;
}
