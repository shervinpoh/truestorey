...
export const JOBS = [
   ...
   * budget and quantum all read hdb.json and private.json — the two files this job
   * replaces — so leaving them out is the same bug the note above records,
   * with these derived files in it: Blindspot would score against last month's
  ...
  { key: 'transactions', file: 'index.json', every: 1,
    cmd: 'npm run ingest:hdb && npm run ingest:ura && npm run index'
       ...
       + ' && npm run build:comps && npm run build:trend && npm run build:quantum'
       + ' && npm run build:budget && npm run build:mop-filings',
  ...
  /* MOE publishes one exercise a year and replaces the last. Checked monthly
     so a new year arrives within a month of MOE posting it; the ingest keeps
     every year it has seen. */
  { key: 'p1', file: 'p1.json', every: 30, cmd: 'npm run ingest:p1',
    why: 'MOE publishes each Primary 1 exercise once, after it ends' },
  ...
  /* build:breakeven reads the awards — its land-cost model is fitted on them —
     so it is rebuilt with them, or it would describe last month's tenders. */
  { key: 'gls-awards', file: 'gls-awards.json', every: 3, cmd: 'npm run ingest:gls-awards && npm run build:breakeven',
