...
/**
 * One article by id, whatever its status. Service role: this is the read the
 * publish gate makes before it lets a draft out, so it must see drafts.
 */
export async function articleById(id) {
  const { data, error } = await request(
    `articles?select=${COLUMNS}&id=eq.${encodeURIComponent(id)}&limit=1`, { service: true });
  return error ? null : (data?.[0] || null);
}

/** Titles filed recently, for the duplicate check. Any status: a story already
    sitting unpublished in the queue is still a story we have covered. The
    header image rides along so intake can avoid a photograph already in use. */
export async function recentTitles({ limit = 60 } = {}) {
  const { data, error } = await request(
    `articles?select=id,slug,title,status,created_at,header_image_url&order=created_at.desc&limit=${limit}`,
    { service: true });
  return error ? [] : (data || []);
}
 ...
 */
/**
 * ── THE SERVER RETURNS AT MOST 1,000 ROWS, WHATEVER `limit` SAYS ───────────
 * This asked for `limit=20000` in one request. PostgREST caps a response at
 * its max-rows setting — 1,000 on Supabase — and says nothing, so `npm run
 * stats -- all` printed "1,000 events · 110 sessions" over a table holding
 * 2,043 and 245. Half the site's traffic, and the report said "all".
 *
 * So it pages, on `id` rather than an offset: ids only grow, so an event
 * written while the report is reading cannot shift a page boundary and be
 * counted twice or skipped, which an offset over a live table can do.
 */
...
export async function recentEvents({ limit = 5000 } = {}) {
  ...
  while (rows.length < limit) {
    const take = Math.min(EVENT_PAGE, limit - rows.length);
    const { data, error } = await request(
      `events?select=id,t,e,s,props&order=id.desc&limit=${take}${before ? `&id=lt.${before}` : ''}`,
      { service: true });
  ...
  return { rows: rows.map(r => ({ t: r.t, e: r.e, s: r.s, ...(r.props || {}) })), error: null };
}
