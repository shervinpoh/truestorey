 ...
 */
export function errorFor({ kind, bandPct, effN, ownShare } = {}) {
  const t = table();
  if (!t) return null;
  if (!Number.isFinite(bandPct) || !Number.isFinite(effN)) return null;

  /* Evidence carried mostly by OTHER buildings first — see OWN_CUTS in
     scripts/build-avm-error.mjs. Neither effN nor the band sees it: a lookup
     can weigh five good sales and have four of them come from next door. */
  const cuts = t.cuts?.own;
  let ownKey = null;
  if (Array.isArray(cuts) && Number.isFinite(ownShare)) {
    if (ownShare < cuts[0]) ownKey = 'lowOwn';
    else if (ownShare < cuts[1]) ownKey = 'partOwn';
  }
  const ownHit = ownKey ? t.own?.[kind]?.[ownKey] : null;

  /* Then thin evidence: a lookup weighing about one sale is graded against
     lookups that were just as thin — see THIN_EFFN. */
  const thinHit = !ownHit && Number.isFinite(t.cuts?.thinEffN) && effN <= t.cuts.thinEffN ? t.thin?.[kind] : null;
  const key = ownHit ? `${kind}|${ownKey}`
    : thinHit ? `${kind}|thin`
    : `${kind}|b${binOf(bandPct, t.cuts.bandPct)}|n${binOf(effN, t.cuts.effN)}`;
  const bin = ownHit || thinHit || t.bins[key];
