...
export function errorFor({ kind, bandPct, effN } = {}) {
  ...
  /* Thin evidence first: a lookup weighing about one sale is graded against
     lookups that were just as thin — see THIN_EFFN in
     scripts/build-avm-error.mjs for why the tercile bin was not enough. */
  const thinHit = Number.isFinite(t.cuts?.thinEffN) && effN <= t.cuts.thinEffN ? t.thin?.[kind] : null;
  const key = thinHit ? `${kind}|thin` : `${kind}|b${binOf(bandPct, t.cuts.bandPct)}|n${binOf(effN, t.cuts.effN)}`;
  const bin = thinHit || t.bins[key];
