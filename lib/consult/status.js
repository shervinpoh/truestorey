...
export function dataStatus({ root = process.cwd(), now = new Date() } = {}) {
  ...
  /** Built before any of its input files last changed on disk. */
  const builtBefore = (builtAt, ...inputs) => inputs.some(n => {
    const m = mtime(n);
    return m && builtAt && new Date(builtAt) < new Date(m);
  });
  const items = [];
  ...
  items.push({
    ...
    /* It only moves when a new launch files its first sales or a new site is
       ...
       pipeline table. */
    /* Stale when built before the files it reads were last refreshed — the
       same test the island scan uses, and the right one: age says nothing
       about whether a new launch or a new award is missing from it. */
    stale: !B || builtBefore(B.builtAt, 'private.json', 'gls-awards.json'), refresh: 'npm run build:breakeven',
  });
  ...
  items.push({
    ...
    /* Rebuilt from the same private.json the AVM reads, so it goes stale on
       the same clock. */
    /* The valuation reads each building's level from this file, so a scan
       built before the transactions it describes is quietly out of step with
       every private estimate. */
    stale: !PS || builtBefore(PS.builtAt, 'private.json'), refresh: 'npm run build:private-scan',
  });
