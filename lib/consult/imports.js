...
export function sizeCheck(rec, areaSqft, statedType = null) {
  ...
  const type = typeFromListing(statedType) || (types.length === 1 ? types[0] : null);
  if (!type) {
    return { ran: false, why: `This block files ${types.length} flat types and the listing does not say which, so there is no single size to compare against.` };
  }
  return { ...checkArea({ areaSqft, sales, type }), type };
}
...
export function rebuildRealis({ root } = {}) {
  const P = paths(root);
  const files = fs.existsSync(P.realisDir) ? fs.readdirSync(P.realisDir).filter(f => /\.csv$/i.test(f)).sort() : [];
  if (!files.length) return { ok: false, reason: 'No REALIS exports yet.' };
