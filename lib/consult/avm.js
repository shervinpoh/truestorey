...
export function estimate(rec, { areaSqft, floor = null, asOf = new Date(), months = 12, min = 5, useLevels = true,
  ...
  const err = errorFor({ kind: rec.kind, bandPct: rawBandPct, effN: effectiveN });
  ...
  return {
    ...
    band: {
      ...
      outer: Number.isFinite(err?.p90Pct) ? {
        psfLow: Math.round(point * (1 - err.p90Pct)),
        psfHigh: Math.round(point * (1 + err.p90Pct)),
        priceLow: Math.round(point * (1 - err.p90Pct) * sqft),
        priceHigh: Math.round(point * (1 + err.p90Pct) * sqft),
      } : null,
