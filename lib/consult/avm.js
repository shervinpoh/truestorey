...
/* v7: nearby private comparables restated to this building's measured level,
   and a band that is never narrower than the method's own typical miss. The
   error table built against v6 describes a method that no longer runs. */
export const VERSION = '2026-09-avm-v7';
 ...
 */
/* One index per field. `gap` is the level over the whole window and is what
   runs live. `earlyGap` is the level as measured in the FIRST third of the
   window only — about three years before the sales the backtest prices — and
   exists so the adjustment can be tested with nothing borrowed from the
   period it is graded on. */
let LEVELS = new Map();
function levelIndex(root = process.cwd(), field = 'gap') {
  if (LEVELS.has(field)) return LEVELS.get(field);
  const idx = new Map();
  LEVELS.set(field, idx);
  try {
    ...
    for (const p of raw.projects) {
      if (Number.isFinite(p[field])) idx.set(normName(p.project) + '|' + p.district, p[field]);
    }
  } catch { /* not built — every lookup misses and nothing is adjusted */ }
  return idx;
}
/** Tests rebuild the file underneath us; nothing else needs this. */
export function _clearLevels() { LEVELS = new Map(); }
const normName = s => String(s || '').toUpperCase().replace(/[^A-Z0-9]+/g, ' ').trim();

export function estimate(rec, { areaSqft, floor = null, asOf = new Date(), months = 12, min = 5, useLevels = true, levelField = 'gap',
  ...
  const levels = useLevels && rec.kind !== 'HDB' ? levelIndex(process.cwd(), levelField) : new Map();
  ...
  /* Keyed on the RAW spread of the evidence — the quantity the error table
     was built on. The published band below may be widened; feeding the
     widened width back in would move the lookup into a bin that describes a
     different shape of evidence. */
  const rawBandPct = (p75 - p25) / point;
  /* How much of the answer is this building's own sales, by weight. Needed
     BEFORE the error lookup: across held-out private sales, lookups carried
     mostly by neighbours missed by far more than their bin promised — the
     table said "nine in ten within 13.8%" and two in three were. */
  const ownShare = (() => {
  ...
  const err = errorFor({ kind: rec.kind, bandPct: rawBandPct, effN: effectiveN, ownShare });

  /**
   * ── A RANGE IS NEVER NARROWER THAN THE METHOD'S OWN TYPICAL MISS ────────
   * The band is the weighted interquartile range of the comparables. When
   * one sale carries most of the weight, the 25th, 50th and 75th percentiles
   * all land on it and the "range" is a single figure: PARC CLEMATIS at 1,044
   * sqft printed "2,490–2,490 psf · S$2,599,560 – S$2,599,560", with one sale
   * holding 90% of the weight. Across private lookups, 3.6% collapsed to one
   * number and another 9.6% came out under 2% wide.
   *
   * A zero-width range is not a precise answer. It is the absence of any
   * measurement of spread, printed as though it were the most confident
   * reading in the tool — and it is a single valuation number, which rule 2
   * keeps off anything a client sees.
   *
   * So each side sits at least the measured median miss away from the point,
   * taken from the same held-out table that reports the error below. That is
   * the matching coverage: the interquartile range holds the middle half of
   * the evidence; the median miss is the level half of lookups like this come
   * in under. Where the evidence is already wider, it is left alone.
   */
  const floorPct = Number.isFinite(err?.medianPct) ? err.medianPct : null;
  let lowPsf = p25, highPsf = p75;
  if (floorPct !== null) {
    lowPsf = Math.min(p25, point * (1 - floorPct));
    highPsf = Math.max(p75, point * (1 + floorPct));
  }
  const widened = lowPsf < p25 || highPsf > p75;
  ...
  return {
     ...
     */
    band: {
      psfLow: Math.round(lowPsf),
      psfHigh: Math.round(highPsf),
      priceLow: Math.round(lowPsf * sqft),
      priceHigh: Math.round(highPsf * sqft),
      widened,
      /** What the evidence alone said, kept so the widening is visible and
       *  so the error table keeps reading the quantity it was built on. */
      evidencePsfLow: Math.round(p25),
      evidencePsfHigh: Math.round(p75),
      rawPct: Number(rawBandPct.toFixed(4)),
      /**
       * Where nine in ten lookups shaped like this one have landed: the point
       * plus and minus the measured p90 miss, from the same held-out table.
       * The p90 calibrates at 90.2% on 3,647 lookups the table never saw, so
       * this is a claim that has been checked rather than a margin chosen to
       * look careful. Printed in dollars because nobody can take 13.8% of
       * S$2.6m in their head while a client is waiting.
       */
      /* And it always CONTAINS the range above. Where the comparables are
         more widely spread than the method's p90 miss, point ± p90 comes out
         narrower than the half-of-sales range, and the report printed
         "half land in S$1.78m–S$2.45m; nine in ten in S$1.86m–S$2.53m" for
         Sceneca Residence — one lookup in five, incoherent on its face. It
         takes whichever edge is further out, which makes the nine-in-ten
         claim conservative there rather than wrong. */
      outer: Number.isFinite(err?.p90Pct) ? (() => {
        const lo = Math.min(point * (1 - err.p90Pct), lowPsf);
        const hi = Math.max(point * (1 + err.p90Pct), highPsf);
        return {
          psfLow: Math.round(lo), psfHigh: Math.round(hi),
          priceLow: Math.round(lo * sqft), priceHigh: Math.round(hi * sqft),
          extendedByEvidence: lo < point * (1 - err.p90Pct) || hi > point * (1 + err.p90Pct),
        };
      })() : null,
      /** Collapsed with no error table to widen it by. Printed as a warning,
       *  never as a precise answer. */
      collapsed: !widened && Math.round(p25) === Math.round(p75),
      basis: widened
        ? `widened to the method's measured typical miss of ${(100 * floorPct).toFixed(1)}% either side — the comparables alone could not define a spread`
        : 'weighted interquartile range of the adjusted comparables',
    },
