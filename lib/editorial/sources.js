...
/** The site a URA sale-site release is about: "… sale site at X" → "x". */
const siteOf = title => String(title).replace(/^.*?sale site at /i, '').trim().toLowerCase();

/**
 * The release to write about today, or null.
 *
 * ── WHY THE AWARD CHECK READS EVERYTHING FOUND ─────────────────────────────
 * A tender closing is only worth a piece until its award is out. That check
 * used to look for the award among the releases NOT yet written about — so
 * once the award had its own piece, it dropped out of the list, the check
 * found no award, and the closing was written up as news. On 26 Sep 2026 that
 * filed "7 bids for Lorong Puntong land, and still no award" eight days after
 * the award (pr26-66) had been published and written up here. An award
 * supersedes its closing whether or not it has been covered.
 */
export function chooseRelease({ found = [], covered = new Set(), filed = [], duplicateOf = () => null } = {}) {
  const fresh = found.filter(r => !covered.has(r.url)
    && !duplicateOf({ title: r.title, slug: r.title.toLowerCase().replace(/[^a-z0-9]+/g, '-') }, filed));
  const awarded = new Set(found.filter(r => /tender award|award of tender/i.test(r.title)).map(r => siteOf(r.title)));
  const pool = fresh.filter(r => !(/tender closing/i.test(r.title) && awarded.has(siteOf(r.title))));
  pool.sort((a, b) => weight(b.title) - weight(a.title) || String(b.iso).localeCompare(String(a.iso)));
  return pool[0] || null;
}
