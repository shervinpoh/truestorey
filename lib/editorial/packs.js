...
export function releasePack(release, { site }) {
  ...
  if (land) {
    ...
    /* Cited as a source too, not only linked: the desk skips a finding whose
       page a recent article cites, and an award the news desk has written
       up would otherwise come back the next morning as the desk's land-award
       finding — the same story twice. */
    sources.push(`${site}/land`);
  }
