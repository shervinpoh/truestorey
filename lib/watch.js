...
/**
 * PAUSED, 26 Sep 2026 — Shervin: "let's scrap this whole thing for now".
 *
 * One switch, read in three places: RecordPage stops offering the form,
 * /api/watch refuses a new watch before reading or writing anything, and
 * scripts/send-digest.mjs sends nothing and moves no watermark. The confirm
 * and unsubscribe links keep working — an address already on the list must
 * always be able to leave it. Everything else is intact; flip this to resume.
 *
 * On resuming: watermarks did not move while paused, so the first digest
 * reports everything filed since each watch's last one. Re-baseline first
 * (a --dry run shows what would go) if that is more than a reader wants.
 */
export const WATCH_PAUSED = true;
