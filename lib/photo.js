...
export const SUBJECTS = [
  ...
  { id: 'bto', lane: 'thing',
    ...
    queries: ['construction crane', 'building under construction', 'scaffolding'] },

  { id: 'land', lane: 'thing',
    when: /\bgls\b|government land sales|land tender|tender clos|land parcel|\baward/,
    queries: ['empty plot of land', 'construction site fence', 'surveying equipment'] },

  { id: 'enbloc', lane: 'thing',
    when: /en[- ]bloc|collective sale|\bstrata\b|redevelop/,
    queries: ['moving boxes', 'demolition site', 'empty apartment balcony'] },

  { id: 'launch', lane: 'thing',
    when: /new launch|showflat|show flat|developer|launch price|uncompleted/,
    queries: ['architectural model', 'floor plan drawing', 'blueprint'] },

  { id: 'tax', lane: 'thing',
    when: /absd|bsd|stamp duty|decoupl|99[- ]to[- ]1|loophole|tdsr|\bmsr\b|cooling measure/,
    queries: ['signing a contract', 'calculator and paperwork', 'fountain pen on document'] },

  { id: 'lease', lane: 'thing',
    when: /leasehold|freehold|99[- ]year|lease decay|\btenure\b|remaining lease/,
    queries: ['house keys', 'old door lock', 'weathered concrete wall'] },

  { id: 'rent', lane: 'thing',
    when: /\brent\b|rental|landlord|tenant|tenancy|yield/,
    queries: ['apartment keys on a table', 'empty apartment interior', 'lease agreement'] },

  { id: 'floor', lane: 'thing',
    when: /high floor|low floor|floor premium|\bstorey\b|\bstack\b|unblocked/,
    queries: ['view from a high window', 'apartment window looking out', 'balcony railing'] },

  { id: 'money', lane: 'thing',
    when: /\bloan\b|mortgage|interest rate|afford|downpayment|\bcpf\b|refinanc/,
    queries: ['calculator and paperwork', 'coins stacked', 'household budget notebook'] },
