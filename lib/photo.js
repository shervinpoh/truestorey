...
/* Ordered: the first match wins, so the specific sit above the general. A
   ...
   ── AND A QUERY IS A GUESS UNTIL SOMEBODY LOOKS AT THE RESULTS ───────────
   Four of these were wrong on the first draft and all four looked fine in the
   table: "surveying equipment" returned a hay baler and a snow plough,
   "weathered concrete wall" a flat grey swatch, "floor plan drawing" two
   auditoriums, "empty plot of land" nothing at all. None of that is visible
   from here. Change a query and look at what it returns before shipping it —
   backfill:photos --only=<slug> exists to make that cheap. */
export const SUBJECTS = [
  ...
  { id: 'land', lane: 'thing',
    ...
    /* Not "surveying equipment": it returns agricultural and municipal kit —
       a hay baler in a barn, and a pair of loaders fitted with snow ploughs,
       both for a Singapore land tender. Named machines return the machine. */
    queries: ['excavator', 'bulldozer', 'wheel loader'] },
...
/**
 * The photo's own identity, not the URL it arrived on. Unsplash varies the
 * query string per request — ixid changes every time — so two articles can
 * carry the same photograph behind two URLs that do not match as strings.
 * That is how the two new-launch pieces ended up with one floor plan between
 * them while a URL comparison reported fifteen distinct images.
 */
export const photoId = url => (/photo-([a-z0-9-]+)/i.exec(String(url || '')) || [])[1] || null;
 ...
 *
 * @param {Set<string>} avoid  photo ids already in use elsewhere. Two
 *   articles side by side with the same picture reads as a broken page, and
 *   a subject with three queries collides more often than it looks.
 *
 * A proper noun in either is only ever matched against, never searched for.
 */
export async function photograph(about = '', also = '', avoid = new Set()) {
  ...
  try {
    for (const [s, query] of attempts) {
      ...
      const qualified = qualify(results, s.lane);
      const ok = qualified.filter(p => !avoid.has(photoId(p.urls?.regular)));
      if (!ok.length) {
        console.warn(`  nothing usable in "${query}" (${s.lane} lane, ${results.length} results`
          + `${qualified.length ? `, ${qualified.length} already in use` : ''})`);
        continue;
      }
      const p = pick(ok);
