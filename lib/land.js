...
export function indexProjects({ condo = [], landed = [] } = {}) {
  ...
  for (const r of [...condo, ...landed]) {
    ...
    /* A street record answers to its own name AND to the named estates on it.
       Landed houses are addressed by street, so "Gabriel Villas" is no longer a
       record of its own — but a GLS site was awarded under that name and has to
       resolve to somewhere. The estate names come from the sales themselves, so
       this is not a second hand-kept table.

       Estate keys go through the same collision rule as everything else: if two
       records claim one key it is dropped rather than guessed at. */
    const names = [String(r.label).replace(/^Landed\s*·\s*/i, ''), ...(r.estates || [])];
