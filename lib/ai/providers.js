 ...
 *
 * `search` is merged into the request body for the retrieval-shaping fields —
 * `web_search_options`, `search_domain_filter`, `search_recency_filter`. They
 * are passed through rather than wrapped because they are the provider's own
 * vocabulary and inventing a second one here would only mean two things to keep
 * in step. Verified accepted against the live API on 30 Aug 2026; an unknown
 * field is rejected with a 400, not ignored, so a rename will be loud.
 */
export async function perplexityStream(messages, { model = 'sonar', maxTokens = 900, search, signal } = {}) {
  const key = process.env.PERPLEXITY_API_KEY;
  if (!key) return null;
  return fetch('https://api.perplexity.ai/chat/completions', {
    ...
    headers: { Authorization: `Bearer ${key}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({
      model, messages, max_tokens: maxTokens, temperature: 0.2, stream: true, ...(search || {}),
    }),
    signal,
  });
}

/* The 5-series models refuse `temperature` outright — "`temperature` is
   deprecated for this model", HTTP 400 — where the 4-series take it. Found by
   calling them, 26 Sep, the same way the Make blueprint found it for Opus 5. */
const takesTemperature = model => /-(3|4)(-|$)|-4-\d/.test(model);

/** Claude, for synthesis. Returns { text } or null. */
export async function claude(system, messages, { model = 'claude-sonnet-4-6', maxTokens = 1400, signal, timeoutMs = 45_000, effort = null } = {}) {
  const key = process.env.ANTHROPIC_API_KEY;
  if (!key) return null;
  const t = timeout(timeoutMs, signal);
  try {
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ model, max_tokens: maxTokens, system, messages,
        ...(takesTemperature(model) ? { temperature: 0.2 } : {}),
        /* Opus 5.5 thinks adaptively and refuses thinking:disabled; effort is
           how its thinking is bounded. Low halves a floor plan reading. */
