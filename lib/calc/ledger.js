...
import { bsd, absd, calendarMonthOffset, ssd } from './stampDuty.js';
...
/**
 * What a sale at a given price actually leaves, once everything is settled.
 *
 * ── WHY THIS IS NOT breakEven() RUN BACKWARDS ──────────────────────────────
 * breakEven() answers "what price returns my money". This answers the harder
 * one: at a price that does NOT, where does the shortfall land? Those are not
 * the same question, because the money is not all owed to the same people and
 * the rules for falling short differ by creditor:
 *
 *   - The BANK must be paid in full. A sale that does not redeem the loan
 *     needs cash brought to completion, and that is money the seller has to
 *     find on the day.
 *   - CPF takes what is left, and at a genuine market-value sale it WAIVES
 *     the rest. That shortfall is real — it is retirement money that never
 *     goes back — but it is not cash anyone has to produce.
 *
 * Collapsing those two into one "loss" figure is the mistake that makes a
 * downside model either alarmist or useless. They are returned separately and
 * the page labels them separately.
 *
 * ── THE FLOOR THAT WAS A BUG ONCE ──────────────────────────────────────────
 * `Math.max(0, …)` around a seller's proceeds is how this repo already told a
 * seller who owed S$197,747 at completion that they walked away with nothing.
 * The zero here is deliberate and different: `toSeller` is floored because you
 * cannot receive negative money, and the amount that would have been negative
 * is returned in full as `cashToComplete` rather than discarded.
 */
export function saleOutcome({
  salePrice, outstanding = 0, cpfRefund = 0, legalSell = 0,
  agentRate = 0, ssdRate = 0, cashIn = 0,
}) {
  const price = Math.max(0, Number(salePrice) || 0);
  const selling = price * (agentRate + ssdRate) + legalSell;
  const afterCosts = price - selling;
  const afterLoan = afterCosts - outstanding;

  const cashToComplete = afterLoan < 0 ? -afterLoan : 0;
  const available = Math.max(0, afterLoan);
  const cpfRefunded = Math.min(cpfRefund, available);
  const toSeller = Math.max(0, available - cpfRefund);

  return {
    salePrice: Math.round(price),
    sellingCosts: Math.round(selling),
    outstanding: Math.round(outstanding),
    /** Money that must be found on completion day. The bank is not optional. */
    cashToComplete: Math.round(cashToComplete),
    cpfRefunded: Math.round(cpfRefunded),
    /** Never refunded to the Ordinary Account. Waived at market value, and
     *  still gone from a retirement account for good. */
    cpfShortfall: Math.round(cpfRefund - cpfRefunded),
    toSeller: Math.round(toSeller),
    /** Cash in hand at the end against cash put in over the whole hold.
     *  Negative is a loss of the reader's own money, in their own pocket. */
    cashChange: Math.round(toSeller - cashToComplete - cashIn),
  };
}

/**
 * The sale at the price a sale must clear, laid out as a completion statement:
 * the price, what comes off it in the order a conveyancing lawyer's statement
 * takes it, and what reaches the seller.
 *
 * ── WHY THE LEDGER ENDS HERE ───────────────────────────────────────────────
 * The page's headline is "a sale must clear S$X". The ledger used to list the
 * pieces of that figure in groups and never put them back together, so it
 * read as an appendix to the number rather than the working behind it
 * (Shervin, 26 Sep: "it feels like an unwanted piece of information"). This
 * is the working: at that price, these come off, and this is left.
 *
 * Each line is rounded as it will be printed and the last line is the
 * arithmetic of the printed lines, so the column adds up exactly as shown. It
 * lands within a dollar or two of the cash put in, because breakEven() solves
 * for the price unrounded; a statement that did not add up as printed would
 * cost more trust than the dollar.
 *
 * Null when there is no break-even price to lay out.
 */
export function saleStatement(r) {
  const price = r?.breakEven?.returnOfCash;
  if (!price) return null;
  const lines = [
    { key: 'loan', amount: r.holding.outstanding },
    ...(r.cpf.total > 0 ? [{ key: 'cpf', amount: r.cpf.total }] : []),
    { key: 'commission', amount: Math.round(price * r.exit.agentRate) },
    ...(r.exit.ssd.rate ? [{ key: 'ssd', amount: Math.round(price * r.exit.ssd.rate) }] : []),
    { key: 'legal', amount: r.exit.legal },
  ];
  return { price, lines, toSeller: price - lines.reduce((a, l) => a + l.amount, 0) };
}
...
export function ledger({
  ...
  const saleDate = calendarMonthOffset(purchaseDate, months);
  ...
  return {
    ...
    omissions: [
      ...
      'Any change in the property’s value. This ledger is only what it costs to hold, and '
      + 'nothing in it estimates what it is worth or will be worth. What the published '
      + 'index has done over holding periods this length is a separate, dated question — it is '
      + 'the section on being wrong, and it is still not a valuation of this address.',
    ],
    caveats: [
