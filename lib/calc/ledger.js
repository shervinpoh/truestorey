 ...
 */
export function cpfAccrual({ lump = 0, monthly = 0, months = 0, rate = CPF_OA_RATE }) {
  if (months <= 0) return { principal: lump, interest: 0, total: lump };
  const i = m(rate);
  const growth = Math.pow(1 + i, months);
  const onLump = lump * (growth - 1);
  // FV of the stream minus what was paid in — i.e. the interest part alone.
  const onStream = monthly ? monthly * ((growth - 1) / i - months) : 0;
  const principal = lump + monthly * months;
  const interest = onLump + onStream;
  return { principal, interest, total: principal + interest };
}

/** What a stream of cash would be worth at a given rate. Same shape, no split. */
export function futureValue({ lump = 0, monthly = 0, months = 0, rate = CPF_OA_RATE }) {
  if (months <= 0) return lump;
  const i = m(rate);
  const growth = Math.pow(1 + i, months);
  return lump * growth + (monthly ? monthly * ((growth - 1) / i) : 0);
}
...
export function ledger({
  ...
  /* No loan means no instalment, so there is nothing for CPF to service — an
   * outright purchase must not go on accruing monthly OA contributions against
   * a mortgage that does not exist. An earlier `instalment || cpfMonthly`
   * fallback did exactly that, and only showed up when the price slider went
   * low enough for the down payments to cover the whole thing. */
  const cpfPerMonth = loan > 0 && instalment > 0 ? Math.min(cpfMonthly, instalment) : 0;
  const cpf = cpfAccrual({ lump: cpfDown, monthly: cpfPerMonth, months });
  ...
  const cashPerMonth = Math.max(0, (instalment || 0) - cpfPerMonth);
  const cashIn = cashAtEntry + cashPerMonth * months;
  ...
  const cashBaseline = futureValue({ lump: cashAtEntry, monthly: cashPerMonth, months, rate: CPF_OA_RATE });
  const cpfBaseline = breakEven({ target: cashBaseline, ...common });

  return {
    price, purchaseDate, propertyType, yearsHeld, months,
    entry,
    holding: { instalment: Math.round(instalment), interestPaid: Math.round(interestPaid), outstanding: Math.round(outstanding), impossible },
