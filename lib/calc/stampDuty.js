...
/**
 * bsd()'s bands as they can be printed under its total.
 *
 * Every full band's duty is a whole dollar; only the top, partial band can
 * carry cents, and bsd() rounds the total to the dollar. Printed as they come,
 * the column can sit a dollar off its own total at a price like S$1,234,567.
 * So the top band is printed as the total less the bands above it — the
 * total's own rounding, applied where it falls — and the column adds up to the
 * total it sits under.
 */
export function bsdBandsAsPrinted(b) {
  const bands = b?.bands || [];
  const above = bands.slice(0, -1).reduce((s, x) => s + x.duty, 0);
  return bands.map((x, i) => ({ ...x, duty: i < bands.length - 1 ? x.duty : b.total - above }));
}
