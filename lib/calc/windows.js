 ...
 */
/**
 * The bar is four INDEPENDENT stretches, not four windows.
 *
 * ── WHY THE WINDOW COUNT IS NOT A SAMPLE SIZE ──────────────────────────────
 * Every window here overlaps its neighbour by all but one quarter. Over a
 * 17-year holding period that is 99% shared, and URA's 51 years of index
 * contain exactly 3.0 non-overlapping 17-year stretches. The page was
 * reporting 138.
 *
 * That is not a rounding problem, it is the whole reading. At 17 years more
 * than half the windows start before 1996 and inherit the run-up from an index
 * that stood at 10.5 in 1975 — so the median came out at +114%, the 90th
 * percentile at +492%, and the best at +818.8%, which turned a S$1.6m home
 * into S$14.7m and made the section read as a fantasy. No choice of statistic
 * fixes it, because the contamination is in the middle of the distribution and
 * not only in its tail.
 *
 * Four is not a new threshold — it is the one already applied to the window
 * count directly below, moved onto the quantity that was supposed to be
 * measured. It puts the cut at just under 13 years on this index and earlier
 * on any shorter one, which is the point: a shorter series should refuse
 * sooner. A check that cannot run scores nothing and says so.
 */
const MIN_INDEPENDENT = 4;
...
export function distribution(series, years) {
  ...
  const independent = (series?.values?.length || 0) / quarters;
  if (independent < MIN_INDEPENDENT) return null;
  ...
  return {
    ...
    /** How many of those windows do not overlap. The honest sample size, and
        the figure the page cites when it says these are readings and not
        trials. */
    independent: Math.floor(independent),
 ...
 */
/**
 * A total change over n years, expressed as the annual rate that produces it,
 * and back again.
 *
 * ── WHY BOTH DIRECTIONS EXIST ──────────────────────────────────────────────
 * A reader thinks in "2% a year". The record is a set of TOTAL changes over
 * whole windows — +30.1% across five years, not a rate. Comparing the two
 * needs one conversion, and doing it in the component would put the same
 * arithmetic in two places the first time anything else needs it.
 *
 * Compounding, not division: 2% a year over five years is +10.4%, not +10%,
 * and the gap widens with the horizon. A stress tester that divides is
 * flattering by construction.
 */
export const totalFromCagr = (cagr, years) =>
  (Number.isFinite(cagr) && years > 0 ? Math.pow(1 + cagr, years) - 1 : null);

export function cagrFromTotal(change, years) {
  if (!Number.isFinite(change) || !(years > 0)) return null;
  /* A total loss leaves no rate to report — the root of a negative base is not
     a real number, and returning NaN dressed as a percentage would be worse
     than saying nothing. */
  if (1 + change <= 0) return null;
