...
export function ogForPost(post) {
  if (!post) return null;
  /* ── AN ARTICLE SHARES WITH ITS PHOTOGRAPH ─────────────────────────────
     The card put the publication date in the big-figure slot, so a note
     forwarded into a chat arrived as "2026-09-06" in 116px type. A piece with
     a photograph of its place now shares with that photograph, and the
     credit travels printed on the card: CC BY asks for attribution wherever
     the photograph appears, and a chat preview is somewhere it appears. */
  if (post.cover?.src) {
    return '/og?' + q({
      a: 1,
      t: post.title,
      s: post.summary,
      k: `${post.kind === 'deep' ? 'Deep dive' : 'Note'} · ${post.date}`,
      img: post.cover.src,
      cr: `Photo: ${post.cover.by} · ${post.cover.licence} · Wikimedia Commons`,
    });
  }
  return '/og?' + q({
    t: post.title,
    ...
    s: post.summary,
    k: `${post.kind === 'deep' ? 'Deep dive' : 'Note'} · ${post.date}`,
    src: 'Written against the filed data · truestorey.vercel.app',
  });
}
