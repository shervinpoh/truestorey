...
export const CHECKS = {
  ...
  view: {
    key: 'view',
    max: 2,
    title: 'What the zoning permits nearby',
    needs: 'URA Master Plan land use and plot ratio.',
    source: 'URA Master Plan 2025 Land Use Layer via data.gov.sg',
    /**
     * The highest plot ratio the Master Plan permits within 300m.
     *
     * THE TITLE CHANGED AND THE CLAIM SHRANK WITH IT. This was "what could be
     * built next door", measured over undeveloped land — which required knowing
     * which parcels are empty. The layer does not carry that: it has LU_DESC,
     * GPR and shape metrics, and zoning is a permission rather than an
     * observation. Inferring vacancy from a zoning category would be inventing
     * the one fact the check turned on. See scripts/ingest-zoning.mjs.
     *
     * So this now says what it can defend: what the plan ALLOWS nearby. A tall
     * permission is a real thing to know before committing, and it flags a
     * possibility — never a prediction that anything will be built.
     */
    bands: [
      { over: 2.79, points: 2, say: r => `Land within 300m is zoned to a plot ratio of ${r} — tall enough to change what you look at.` },
      { over: 1.39, points: 1, say: r => `Land within 300m is zoned to a plot ratio of ${r}.` },
      { over: -1,  points: 0, say: r => r ? `Nothing within 300m is zoned above a plot ratio of ${r}.` : 'No parcel within 300m carries a fixed plot ratio.' },
    ],
    /**
     * Most of a typical 300m radius carries no fixed ratio — landed housing is
     * governed by storey height, and some parcels are left open by the plan.
     * Reporting only the highest fixed figure would read as a ceiling when it
     * is the highest of a minority.
     */
    caveat: (_r, ctx) => {
      if (!ctx || !ctx.parcelsInRadius) return null;
      const { noFixedRatio: n, parcelsInRadius: total } = ctx;
      if (!n) return null;
      return `${n} of the ${total} parcels within ${ctx.m}m carry no fixed plot ratio — `
           + 'landed, or left open by the plan. They are not measured here, and not low.';
    },
  },
};
