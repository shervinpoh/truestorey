 ...
 */
export const RUBRIC_VERSION = '2026-09-v4';
...
/**
 * The five checks. `max` is what a check can contribute when it runs; the
...
export function leasePoints(years) {
  if (!Number.isFinite(years)) return 0;      // freehold, or unknown-but-run
...
const leaseSay = (years, c) => {
  if (!Number.isFinite(years)) return 'Freehold, so there is no lease running down.';
