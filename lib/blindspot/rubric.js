 ...
 * the rest, because two of the six checks — land sales and approvals nearby —
 ...
 */
export const BANDS = [
  { upTo: 0.20, label: 'Little flagged', meaning: 'Nothing in the checks below stood out. That is not the same as no risk — it is no risk of the kinds this tool measures.' },
...
export const CHECKS = {
  ...
  gls: {
     ...
     */
    bands: [
      { over: 800, points: 2, say: (u, c) => {
        const p = programme(c);
        return `${Math.round(u).toLocaleString('en-SG')} units are zoned within 1km, on land already sold or launched`
          + `${p ? ` under the ${p} Government Land Sales programme` : ''}. That is what may be built, not when.`;
      } },
      { over: 200, points: 1, say: (u, c) => {
        const p = programme(c);
        return `${Math.round(u).toLocaleString('en-SG')} units are zoned within 1km, on land`
          + `${p ? ` in the ${p} Government Land Sales programme` : ' in the Government Land Sales programme'}.`
          + ' That is what may be built, not when.';
      } },
      ...
      { over: -1,  points: 0, say: (u, c) => {
        const p = programme(c);
        if (u) {
          return `${Math.round(u).toLocaleString('en-SG')} units zoned within 1km`
            + `${p ? ` on land in the ${p} Government Land Sales programme` : ''} — small.`;
        }
        return `No Government Land Sales site within 1km`
          + `${p ? ` in the ${p} programme` : ' in the programme held here'}.`;
      } },
    ],
    caveat: (_u, c) => {
      if (!c) return null;
...
/* ── A PROGRAMME LABEL IS NOT A DELIVERY DATE ──────────────────────────────
   These sentences used to append a bare "(2026 H2)" to a unit count, and a
   parenthetical takes its meaning from whatever sits in front of it: "140 units
   are coming within 1km from land in the GLS programme (2026 H2)" reads as when
   the units arrive. They do not arrive then. That is the half-year programme
   the LAND was listed in, and land sold in it is years from completion.

   The first repair appended ", listed in the 2026 H2 GLS programme" to every
   band instead, which produced "in the GLS programme, listed in the 2026 H2 GLS
   programme" and, worse, "No GLS site within 1km, listed in the 2026 H2 GLS
   programme" — an absent site being listed. A suffix cannot be correct for four
   different sentences. Each one names the programme in its own words now. */
const programme = c => (c && c.programme ? c.programme : null);
 ...
 *
 * `max` is the sum of the maxima of those checks only. `checks`, `skipped`,
 * and `notApplicable` are separate so the page can say what it could not measure — a tool that
...
export function score(measurements = {}, contexts = {}) {
  ...
  for (const key of Object.keys(CHECKS)) {
    if (contexts[key]?.notApplicable) {
      notApplicable.push({ key, title: CHECKS[key].title, reason: contexts[key].notApplicable });
      continue;
    }
