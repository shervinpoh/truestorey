...
export function glsWithin(lat, lon, { km = 1 } = {}) {
  ...
  return { km, units, sites, source: gls.source, accessedAt: gls.accessedAt };
}
...
export function plotRatioWithin(lat, lon, { m = 300 } = {}) {
  ...
  let highest = 0; let parcel = null;
  for (const p of zoning.parcels) {
    if (!p.undeveloped || !Number.isFinite(p.lat)) continue;
    if (haversine(lat, lon, p.lat, p.lon) > m) continue;
    const r = Number(p.plotRatio) || 0;
    if (r > highest) { highest = r; parcel = p; }
  }
  return { m, plotRatio: highest, parcel, source: zoning.source, accessedAt: zoning.accessedAt };
}
