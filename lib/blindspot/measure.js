...
function sameRecordPrice(rec, askingPsf, type, { months, min, now, areaSqm = null }) {
...
export function leaseFinding(rec, now = new Date()) {
  ...
  if (years === Infinity) {
    return { years: Infinity, freehold: true, relativity: null, decay: null,
             source: 'Tenure as filed with URA' };
  }
...
export function nearbyComps(rec, askingPsf, targetAreaSqm, type, {
  months = 12, min = 5, now = new Date(), index = null,
} = {}) {
...
export function priceAnalysis(rec, askingPsf, {
  areaSqft = null, months = 12, min = 5, now = new Date(), index = null,
} = {}) {
  ...
  const here = sameRecordPrice(rec, askingPsf, type, { months, min, now, areaSqm: targetAreaSqm });
  const wider = here.sufficient ? null
    : sameRecordPrice(rec, askingPsf, type, { months: 24, min, now, areaSqm: targetAreaSqm });
  const nearby = (here.sufficient || wider?.sufficient) ? null
    : nearbyComps(rec, askingPsf, targetAreaSqm, type, { months, min, now, index });
