 ...
 */
export function sizeTrend(rec, targetAreaSqm, { index = null, min = 3 } = {}) {
  ...
  const band = String(Math.floor(targetAreaSqm / (d.bandSqm || 10)) * (d.bandSqm || 10));
  const sized = at[band];
  const all = at.all;
  if (!sized || sized.length < 2 || !all) return null;
  ...
  return {
    band: `${band}\u2013${Number(band) + (d.bandSqm || 10)} sqm`,
    rows, min,
