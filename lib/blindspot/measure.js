...
function applyFloor(rows, floorTo, curve) {
  ...
  const out = rows.map(r => {
    ...
    // `psfFiled` may already be set by a time restatement; the filed figure is
    // the one on the register, not whatever the previous step produced.
    return { ...r, psfFiled: r.psfFiled ?? r.psf, psf: a.psf, floorFrom: a.from };
  });
  /* `basis` travels because it is not a detail: 'within-building' is each
     block measured against itself, 'town bands' is the confounded fallback,
     and a reader deciding how far to trust an adjusted comparable needs to
     know which one moved it. */
  return { rows: out, adjusted: { to: floorTo, scope: curve.scope, basis: curve.basis, where: curve.where, moved, capped, of: rows.length } };
}

/**
 * Restating an older sale to today's price level, by a published index.
 *
 * ── WHY THIS EXISTS ────────────────────────────────────────────────────────
 * Shervin, 25 Sep: a report that cannot determine something should pick up
 * something instead of only saying so. The price check was the gap — measured
 * over 1,000 real addresses it could not run on 2% of HDB blocks, 9% of condos
 * and 27% of landed streets, because a street with three houses sold in two
 * years has no five comparables inside a year. Reaching back to 24 and 36
 * months rescues about half of those.
 *
 * A sale from 2023 is not evidence about 2026 prices as filed, though. Landed
 * prices moved over 20% in that time, so an unadjusted older cohort would put
 * the market's movement into the percentile as though it were about this home.
 * Each older sale is therefore multiplied by the matching official index —
 * HDB's Resale Price Index, or URA's landed or non-landed Private Residential
 * Price Index — from the quarter it sold to the latest quarter published. The
 * page names the index and keeps the filed figure next to the restated one.
 *
 * A sale in a quarter the index does not yet publish is left as filed (factor
 * 1): the index has nothing to say about it. A sale in a quarter the index
 * does not hold at all is dropped rather than guessed at.
 */
const INDEX_FILES = {
  HDB: { file: 'hdb-index.json', key: null, name: 'HDB Resale Price Index' },
  landed: { file: 'ppi.json', key: 'landed', name: 'URA Private Residential Property Price Index — Landed' },
  nonLanded: { file: 'ppi.json', key: 'nonLanded', name: 'URA Private Residential Property Price Index — Non-landed' },
};
const quarterOf = month => {
  const [y, m] = String(month).split('-').map(Number);
  return Number.isFinite(y) && Number.isFinite(m) ? `${y}-Q${Math.ceil(m / 3)}` : null;
};
export function indexAdjuster(kind, landed = false) {
  const spec = INDEX_FILES[kind === 'HDB' ? 'HDB' : landed ? 'landed' : 'nonLanded'];
  const raw = load(spec.file);
  const points = spec.key ? raw?.series?.[spec.key]?.points : raw?.points;
  if (!points?.length) return null;
  const byQuarter = new Map(points.map(p => [p.quarter, p.index]));
  ...
  return {
    ...
    factor(month) {
      ...
      if (!q) return null;
      if (q >= latest.quarter) return 1;
      const at = byQuarter.get(q);
      return Number.isFinite(at) && at > 0 ? latest.index / at : null;
    },
  };
}
...
/**
 * What homes of about this size have been selling for here, year by year —
 * beside what the whole address did.
 *
 * ── WHAT THIS REPLACED, AND WHY ────────────────────────────────────────────
 * The intended feature was realised returns: the same home bought and sold,
 * so a reader could see what one actually made. It cannot be built from this
 * data and the attempt is worth recording. Pairing filed sales needs a unit
 * identifier, and neither HDB nor URA publishes one — deliberately, since
 * unit-level purchase prices are the REALIS-shaped data rule 1 forbids
 * outright. The nearest available match is address plus floor area plus floor
 * band, and it is not a unit: Blk 362C Sembawang Crescent has fifteen 4-room
 * 93 sqm sales on storeys 7 to 9 inside seventeen months, two in the same
 * month. Pairing those would have produced a confident median holding period
 * out of fifteen different families' homes.
 *
 * ── WHAT IS TRUE INSTEAD, AND USEFUL ───────────────────────────────────────
 * A project's headline movement is a median over every size that happened to
 * sell, so it moves when the MIX moves. A quarter heavy on one-bedrooms drags
 * it down with no home anywhere being worth less. Splitting the same filed
 * sales by size shows the reader their own size against the address's
 * headline, and where those two disagree the disagreement is the finding.
 *
 * Years with fewer than `min` sales are returned WITH their count rather than
 * dropped, because a reader can discount a year of two sales and cannot
 * discount one that was silently removed.
 */
export function sizeTrend(rec, targetAreaSqm, { index = null, min = 3, orAllSizes = false } = {}) {
  const d = index || (_trend !== undefined ? _trend : (_trend = load('trend.json', null)));
  const at = d?.records?.[rec?.href];
  if (!at || !Number.isFinite(targetAreaSqm)) return null;

  const wanted = String(Math.floor(targetAreaSqm / (d.bandSqm || 10)) * (d.bandSqm || 10));
  const all = at.all;
  /* A size band with only one year in it is not a trend, and for a young
     project most bands are. `orAllSizes` lets a caller take the whole-address
     series instead of nothing — weaker, because it moves when the MIX of sizes
     sold moves, and the caller must SAY it is every size. Blindspot does not
     ask for this: there the whole point is the reader's own size against the
     address, and a fallback would quietly answer a different question. */
  const usable = at[wanted]?.length >= 2;
  const band = usable ? wanted : (orAllSizes && all?.length >= 2 ? 'all' : null);
  const sized = band ? at[band] : null;
  if (!sized || !all) return null;

  const byYear = new Map(all.map(([y, psf, n]) => [y, { psf, n }]));
  const rows = sized.map(([year, psf, n]) => ({
    year, sizedPsf: psf, sizedN: n,
    allPsf: byYear.get(year)?.psf ?? null, allN: byYear.get(year)?.n ?? 0,
    thin: n < min,
  }));
  const pc = (a, b) => (a && b ? Math.round(((b / a) - 1) * 1000) / 10 : null);
  const first = rows[0], last = rows.at(-1);
  const sizedChange = pc(first.sizedPsf, last.sizedPsf);
  const allChange = pc(first.allPsf, last.allPsf);
  return {
    band: band === 'all' ? 'every size at this address'
      : `${band}\u2013${Number(band) + (d.bandSqm || 10)} sqm`,
    allSizes: band === 'all',
    rows, min,
    from: first.year, to: last.year,
    sizedChange, allChange,
    /** Set when the size and the whole address disagree by more than five
     *  points. That gap is the mix effect, and it is the finding. */
    diverges: sizedChange != null && allChange != null && Math.abs(sizedChange - allChange) > 5,
...
export function storeyCurve(rec, type) {
  ...
  const holder = side.groups?.[key]?.[type];
  if (!holder) return null;

  /**
   * ── WITHIN-BUILDING FIRST ─────────────────────────────────────────────
   * `curve` is each building compared with ITSELF, then pooled across at
   * least five buildings — which removes the estate, the lease, the location
   * and the flat model in one move, because all four are shared by every band
   * of one block. `bands` is the median psf per band across a whole town and
   * is confounded by exactly those things: the blocks tall enough to have a
   * 13th floor are the newer ones, so its upper bands read lease as height.
   *
   * The size of that difference is not subtle. Ang Mo Kio's 4 ROOM bands
   * claim 90.4% from the bottom of the town to the top. Measured inside
   * buildings, floors 1–12 are worth 2.0%.
   *
   * `curve` carries RATIOS around 1.0, not prices. adjustForFloor only ever
   * divides one point by another, so the units cancel and nothing downstream
   * needs to know which basis it got — except the report, which says.
   */
  for (const [basis, rows, isRatio] of [
    ['within-building', holder.curve, true],
    ['town bands', holder.bands, false],
  ]) {
    if (!Array.isArray(rows) || rows.length < 3) continue;
    let points = rows
      .map(([label, mid, value, n]) => ({ mid: Number.isFinite(mid) ? mid : floorMid(label), psf: value, n, label }))
      .filter(p => Number.isFinite(p.mid) && Number.isFinite(p.psf) && p.psf > 0)
      .sort((a, b) => a.mid - b.mid);

    /* ── A BAND THAT JUMPS IS MEASURING VINTAGE, NOT HEIGHT ─────────────
     * Only the town-band fallback can do this, and it is why the fallback is
     * second. A floor is worth roughly 0.5–1.5% in this market, so a
     * three-storey band should step by a few per cent; 28 of the 156 curves
     * this function used to accept carried a step above 25%, and 96 were not
     * monotonic. Truncating at the first offending step keeps the lower bands,
     * where the sample is thick and the signal is real. curveAt() reads flat
     * outside the range, so a comparable above the cut is treated as though it
     * sat at the top of what was measured — which understates its height and
     * never invents a premium. */
    if (!isRatio) {
      const MAX_BAND_STEP = 1.15;
      let cut = points.length;
      for (let i = 1; i < points.length; i++) {
        if (points[i].psf / points[i - 1].psf > MAX_BAND_STEP) { cut = i; break; }
      }
     ...
     * a fact about their home. Where a basis does not rise the next one is
     * tried, and where none does, nothing is adjusted and the page says the
     * comparables are as filed. */
    if (points.length >= 3 && points.at(-1).psf > points[0].psf) {
      return { points, scope: 'local', basis, where: key, type };
    }
  }
  return null;
}
...
export function nearbyComps(rec, askingPsf, targetAreaSqm, type, {
  ...
  /** indexAdjuster(...) — restates each sale to the latest published quarter. */
  adjust = null,
} = {}) {
  ...
  for (const radiusKm of (RADII_KM[hdb ? 'HDB' : 'PRIVATE'])) {
    ...
    for (const [href, o] of Object.entries(all)) {
      ...
      for (const [month, psf, areaSqm, t, floor] of o.sales) {
        ...
        const f = adjust ? adjust.factor(month) : 1;
        if (f == null) continue;
        const row = publicComparable({ month, psf: psf * f, areaSqm, type: t, storey: floor },
          { href, label: o.label, distanceM: Math.round(distanceM) });
        // Every restated row carries its filed figure, even when the two round
        // the same — a flat market gives a factor of 0.9995, and a row that
        // was restated must still say so. The table shows it only if it differs.
        if (adjust && f !== 1) row.psfFiled = Math.round(psf);
        rows.push(row);
        places.add(href);
      }
    }
    ...
    result = priceSummary(on, askingPsf, {
      ...
      restated: adjust ? { index: adjust.name, source: adjust.source, to: adjust.to } : null,
    });
...
export function priceAnalysis(rec, askingPsf, {
  areaSqft = null, floor = null, flatType = null, months = 12, min = 5, now = new Date(), index = null,
  /**
   * Compute the nearby rung even when this address has enough of its own.
   *
   * The ladder exists to STOP at the best available evidence, which is right
   * for a reader placing one price. It is wrong for anything that needs the
   * two cohorts side by side — asking "what do this block's neighbours say it
   * is worth, ignoring the block itself" needs the nearby rung whether or not
   * the block is busy, and it is the busy blocks where the comparison is most
   * worth making. Off by default, so the reader-facing path is unchanged.
   */
  forceNearby = false,
} = {}) {
  ...
  const type = rec.kind === 'HDB' && flatType ? flatType : inferredType(rec, targetAreaSqm);
  ...
  const nearby = (!forceNearby && (here.sufficient || wider?.sufficient)) ? null
    : nearbyComps(rec, askingPsf, targetAreaSqm, type, { months, min, now, index, floorTo, curve });

  /* ── RUNGS 4 AND 5: FURTHER BACK IN TIME, RESTATED BY INDEX ────────────
     Only once everything above has failed, and only for the reader-facing
     path. Each rung widens the radius exactly as rung 3 does. See
     indexAdjuster() for why an older sale is restated before it is compared. */
  let older = null;
  if (!forceNearby && !here.sufficient && !wider?.sufficient && !nearby?.sufficient) {
    const landedType = /terrace|semi|detached/i.test(String(type));
    const adjust = indexAdjuster(rec.kind, landedType);
    if (adjust) {
      for (const back of [24, 36]) {
        older = nearbyComps(rec, askingPsf, targetAreaSqm, type,
          { months: back, min, now, index, floorTo, curve, adjust });
        if (older?.sufficient) break;
      }
    }
  }
  ...
  const scored = here.sufficient ? here
    ...
    : older?.sufficient ? older
    : null;
  const widest = older || nearby || wider || here;
  /* When even the widest search is too thin to score, what it DID find is
     still shown — labelled as too few to score, never ranked. A handful of
     real sales nearby is more use to a reader than the word "unavailable". */
  const thin = !scored && widest?.comparisons?.length ? widest : null;
  ...
  return {
    ...
    floorCurve: curve ? { scope: curve.scope, basis: curve.basis, where: curve.where, points: curve.points.length } : null,
    flatTypeBasis: type
      ? rec.kind === 'HDB' && flatType ? 'the flat type you selected from the listing'
    ...
    unavailable: scored ? null
      : `At least ${min} comparable sales are needed to score the price. This address filed `
        + `${here.sample} in the last ${months} months and ${wider?.sample ?? here.sample} in the last 24`
        + (older ? `, and a search of similar homes within ${Math.round((older.radiusKm || 0) * 1000)}m `
            + `over ${older.months} months found ${older.sample}.`
          : `; the widest nearby cohort found ${nearby?.sample ?? 0}.`),
  };
}
 ...
 */
/**
 * How many storeys URA's own description says a decision is for, or null.
 *
 * The text carries it about 81% of the time — "PROPOSED ERECTION OF A 16-STOREY
 * RESIDENTIAL FLATS". Exported because /insights' sun panel needs the same
 * answer and a second regex over the same field is how two parts of a site
 * start disagreeing about how tall something is.
 *
 * The largest number wins when a description names several: an application
 * covering a 2-storey carpark and a 16-storey block is a 16-storey block. Over
 * 100 is rejected as a misparse — nothing in Singapore is 340 storeys, and
 * "340" in an address will otherwise be read as one.
 */
export function storeysIn(what) {
  const hits = [...String(what || '').matchAll(/(\d+)\s*-?\s*STOREY/gi)]
    .map(x => Number(x[1]))
    .filter(n => n > 0 && n < 100);
...
export function approvalsWithin(lat, lon, { m = 300, years = 3, from = new Date() } = {}) {
  ...
  for (const d of data.decisions) {
    ...
    const max = storeysIn(d.what);
    if (max === null) { unparsed++; continue; }
