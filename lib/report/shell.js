 ...
 *
 * Four reports now render the same shape — a headline figure or two, tables of
 * rows, a list of caveats, sources, a link back and the CEA footer. The rules
 * it enforces are lib/digest.js's, for the reasons
...
export function footer({ link, sources, agent, extraNote = null,
  figureBasis = 'what you entered and from published rates' }) {
  const sourceLine = sources.map(s => `${s.name} (effective ${s.effective})`).join(' · ');
  const text = [
    `THIS IS NOT A VALUATION. Every figure above comes from ${figureBasis}.`,
    'None of it is an opinion about what your home is worth or what it would fetch — that',
    ...
    'Your address was used to send this one email, not added to our contact database or mailing list.',
    'Our email provider received the address and report to deliver it. There is no subscription.',
  ].join('\n');

  const html = `<hr style="${S.rule}">
<p style="margin:0 0 16px;font-size:13.5px;color:#48514F"><strong style="color:#111414">This is not a valuation.</strong> Every figure above comes from ${esc(figureBasis)}. None of it is an opinion about what your home is worth or what it would fetch — that is a separate question, and the filed transaction ranges on the site are the evidence for it.</p>
...
${sourceLine ? `<p style="${S.lab};margin:0 0 4px">${sources.map(s => `${esc(s.name)} (effective ${esc(s.effective)})`).join(' · ')}</p>` : ''}
...
Your address was used to send this one email, not added to our contact database or mailing list. Our email provider received the address and report to deliver it. There is no subscription.
</p>`;
  return { text, html };
}
