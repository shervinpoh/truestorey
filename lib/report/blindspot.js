...
import { unitDetailError } from '../blindspot/unit.js';
...
export function renderBlindspotReport({ values, link, agent, siteUrl, now = new Date() }) {
  ...
  const unitError = unitDetailError(rec, { flatType: values.flatType, bedrooms: values.bedrooms });
  if (unitError) return { error: unitError };
  const r = analyse({
    href: values.home, askPrice: values.price, areaSqft: values.area,
    floor: values.floor || null, flatType: values.flatType || null,
    bedrooms: values.bedrooms || null, now,
  });
  ...
  const asked = [
    ...
    [rec.kind === 'HDB' ? 'HDB flat type supplied' : 'Bedrooms supplied',
      rec.kind === 'HDB' ? r.input.flatType : String(r.input.bedrooms)],
  ...
  const priceLines = [
    ...(rec.kind === 'HDB'
      ? [`The price cohort is matched to the ${r.input.flatType} selected from the listing.`]
      : ['URA sale records do not include bedroom counts. The price cohort uses floor area, property type and tenure, not the bedroom count supplied from the listing.']),
    ...(observed ? [
      `At this address: ${count(observed.sample)} filed sales, ${psf(observed.low)} to ${psf(observed.high)}, ${observed.from} to ${observed.to}.`,
    ] : []),
    ...(scored ? [
      `Scored cohort: ${count(scored.sample)} filed sales${scored.basis === 'nearby' ? ` across ${count(scored.blocks)} nearby ${r.record.kind === 'HDB' ? (scored.blocks === 1 ? 'HDB block' : 'HDB blocks') : (scored.blocks === 1 ? 'project' : 'projects')} within ${count(Math.round(scored.radiusKm * 1000))}m straight-line` : ' at this address'}. Range ${psf(scored.low)} to ${psf(scored.high)}; median ${psf(scored.median)}.`,
      ...(scored.restated ? [`Nothing close enough sold in the last twelve months, so the search went back ${scored.months} months and restated each older sale to ${scored.restated.to} prices using the ${scored.restated.index}. Each comparable below shows its filed figure too.`] : []),
      ...
      `Not scored: ${price.unavailable}`,
      /* The same fallback the page shows: what the widest search found, too
         few to score and never ranked against the asking price. */
      ...(price.thin ? [`What the search did find: ${count(price.thin.sample)} similar sale${price.thin.sample === 1 ? '' : 's'} within ${count(Math.round((price.thin.radiusKm || 0) * 1000))}m over ${price.thin.months} months, ${price.thin.sample === 1 ? `at ${psf(price.thin.low)}` : `from ${psf(price.thin.low)} to ${psf(price.thin.high)}`}${price.thin.restated ? `, restated to ${price.thin.restated.to} prices by the ${price.thin.restated.index}` : ''}. Too few to score, so they add no points; listed below.`] : []),
    ] : []),
    ...(period ? [`${price.source} · ${period.from} to ${period.to} · observed transactions, not a valuation.`] : []),
  ];
  const comps = scored?.comparisons || price?.thin?.comparisons || [];
  const compLines = comps.map(c => `${c.label} · filed ${c.month} · ${c.areaSqm || 'area not held'} sqm${c.storey ? ` · ${c.storey}` : ''} · ${psf(c.psf)}${c.psfFiled != null && c.psfFiled !== c.psf ? ` (filed ${psf(c.psfFiled)})` : ''} · ${c.distanceM ? `${count(c.distanceM)}m straight-line` : 'this address'}`);
  const f = footer({
    link, sources: [], agent,
