 ...
 *
 * The MOE primary school band is the CLOSEST straight-line gets to being the
 * rule's own measure, and it is still not it. MOE measures in a straight line
 * — that part is right — but it measures to the school's LAND BOUNDARY, and
 * this measures to the school's registered coordinate. On a large campus those
 * differ by a couple of hundred metres, which is the whole width of the
 * decision at the edge of a band. See band() below.
 */
 ...
 * within 1km, 1km to 2km, and beyond 2km, measured in a straight line.
 *
 * ── THIS IS AN APPROXIMATION AND USED TO CLAIM IT WAS NOT ──────────────────
 * This header said "so this band is the rule's own measure". The straight line
 * is right and the ENDPOINTS are not: MOE measures from the nearest point of
 * the school's land boundary to the home, and this measures from the school's
 * single registered coordinate. A campus a few hundred metres across therefore
 * reads as further away than MOE would find it, and a home just outside 1km
 * here can be inside 1km there.
 *
 * The official answer is OneMap's SchoolQuery, which returns MOE's own
 * category for an address. This is an indication of which band an address is
 * likely to fall in, and the UI must say so rather than imply a verdict.
 *
