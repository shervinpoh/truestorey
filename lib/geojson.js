...
/**
 * Is this coordinate inside these rings?
 *
 * Ray casting, with the even-odd rule, so a ring inside a ring reads as a hole
 * rather than as solid — which matters here because URA's planning areas carry
 * enclaves, and a point in one would otherwise be assigned to the area
 * surrounding it.
 *
 * Rings are [lon, lat] as GeoJSON stores them, NOT [lat, lon]. Getting that
 * backwards puts every Singapore address in the Indian Ocean and, because the
 * test then fails for everything, looks like an empty dataset rather than a
 * transposed one.
 */
...
/** The first area containing the point, or null. Areas are {name, slug, rings}. */
export function areaAt(lon, lat, areas) {
  for (const a of areas) if (pointInRings(lon, lat, a.rings)) return a;
  return null;
}
