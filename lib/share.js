...
import { HDB_FLAT_TYPES } from './blindspot/unit.js';
...
export const BLINDSPOT_SHARE = {
  ...
  fields: {
    ...
    flatType: { kind: 'enum', values: HDB_FLAT_TYPES },
    bedrooms: { kind: 'int', min: 1, max: 20 },
  },
};

export const BLINDSPOT_LABELS = {
  home: 'property', price: 'asking price', area: 'floor area', floor: 'floor',
  flatType: 'HDB flat type', bedrooms: 'bedrooms',
};

// The first Blindspot links had price, area and floor only. Keep their inputs
// readable, but do not rerun a check until the recipient confirms which unit
// they mean. A version change must not invent that missing detail.
const BLINDSPOT_V1 = {
  v: 1,
  fields: {
    home: BLINDSPOT_SHARE.fields.home,
    price: BLINDSPOT_SHARE.fields.price,
    area: BLINDSPOT_SHARE.fields.area,
    floor: BLINDSPOT_SHARE.fields.floor,
  },
};

export function blindspotShareInput(hash) {
  let got = decodeShare(BLINDSPOT_SHARE, hash);
  const legacy = !got;
  if (!got) got = decodeShare(BLINDSPOT_V1, hash);
  if (!got) return null;
  if (got.dropped.length || !got.values.home || !got.values.price || !got.values.area
    || (!legacy && (!(got.values.flatType || got.values.bedrooms) || (got.values.flatType && got.values.bedrooms)))) {
    return { error: 'This shared check could not be read. Choose the property and enter the listing details again.' };
  }
  return legacy ? { values: got.values, needsUnit: true } : { values: got.values };
}
