...
export function sanitizeHtml(input, { allowDataImages = false } = {}) {
  ...
  while (i < html.length) {
    ...
    // A REAL TAG CANNOT CONTAIN ANOTHER '<'. If this one does, then the '<' we
    // are looking at is a stray bracket in prose and the '>' we found is not
    // its partner — it belongs to a later, genuine tag. Consuming the span
    // between them therefore deletes real text and real markup.
    //
    // This is the prose-eating bug returning by a narrower route. The earlier
    // fix only covered a stray '<' followed by something that cannot start a
    // tag name, which is why "5 < 10" was safe and its test passed. Put a
    // LETTER after the bracket — "price < expected", "a < b" — and the parser
    // reads a plausible tag name, finds it is not on the allowlist, and drops
    // everything to the next '>', closing paragraph included.
    //
    // Escape this one character and rescan from the very next, so the stray
    // bracket is kept as text and the markup after it is parsed properly.
    if (inner.includes('<')) { out += '&lt;'; i = lt + 1; continue; }
