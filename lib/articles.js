import { withEditorialAsset } from './editorial-assets.js';
...
import { readCover } from './cover.js';
...
export function fromRow(row) {
  ...
  return {
    ...
    ...imageOf(row),
    credit: row.unsplash_photographer_name && !readCover(row.header_image_url)
      ? {
...
/**
 * The header image and what may be said about it.
 *
 * A Commons photograph carries its credit in its URL (lib/cover.js). One whose
 * credit cannot be read is not shown at all: the licence allows use WITH
 * attribution, and a page that drops the attribution is a breach on every
 * render, so no picture is the only honest fallback.
 */
function imageOf(row) {
  const cover = readCover(row.header_image_url);
  if (cover?.refused) return { image: null, imageAlt: '', cover: null };
  if (cover) return { image: cover.src, imageAlt: cover.alt, cover };
  return {
...
export async function feed() {
  ...
  if (!configured()) return files.map(withEditorialAsset);
...
export async function piece(slug) {
  ...
  if (file) return withEditorialAsset({ ...file, source: 'file', html: null });
  if (!configured()) return null;
  const row = await articleBySlug(slug);
  return row ? withEditorialAsset(fromRow(row)) : null;
}
