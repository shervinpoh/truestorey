...
export function normaliseReport(raw) {
  ...
  return {
    isFloorPlan: true,
    unit: {
      /* The report's headline. A hedge in brackets belongs in the summary,
         which the prompt asks for; at display size it reads as clutter. */
      type: clean(str(u.type, 120).replace(/\s*\([^)]*\)\s*$/, '')),
