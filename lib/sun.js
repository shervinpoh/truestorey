 ...
 * It says where the sun IS. Whether a window can see it needs footprints and
 * heights; since 26 Sep 2026 data/buildings.json holds them (OpenStreetMap
 * footprints, HDB's storey counts, OSM height tags — lib/massing.js), and
 * lib/sunlight.js does the shading. A building with no published height is
 * still never drawn as a shadow: that would be rule 13, geometry the data
 * does not contain.
 *
 * ── ACCURACY ───────────────────────────────────────────────────────────────
...
/* Singapore's centroid. The island spans about 1.16°N to 1.47°N, and across
   that whole range the sunset bearing differs by 0.029° — measured, not
   assumed, and 0.3% of the narrowest directness band. So a floor plan with no
   address attached can be answered from here without the answer being an
   approximation worth disclosing. A record page still uses its own
   coordinate, because it has one. */
export const SG = { lat: 1.3521, lon: 103.8198 };

/**
 * How the low afternoon sun meets one facing, month by month.
 *
 * For an uploaded floor plan, where the only inputs are a compass reading off
 * the drawing and the fact that the building is in Singapore. Everything else
 * is astronomy: no dataset, no estimate, nothing that can go stale.
 *
 * Returns null for a facing that could not be read, rather than a year of
 * nulls — the page needs to say "the plan carries no north arrow", which is a
 * different sentence from "the sun never reaches it".
 */
export function sunOnFacing(facing, { lat = SG.lat, lon = SG.lon, year } = {}) {
  if (!Number.isFinite(facing)) return null;
  const y = year || new Date().getUTCFullYear();
  const months = sunsetByMonth(lat, lon, y).map(m => {
    const off = offsetFrom(facing, m.azimuth);
    const band = directness(off);
    return { month: m.month, azimuth: m.azimuth, offset: off, band: band?.id, label: band?.label };
  });
  /* The two facts a reader acts on: when it is worst, and whether there is
     any month it misses them entirely. */
  const lit = months.filter(m => m.band && m.band !== 'none');
  const closest = lit.length
    ? lit.reduce((a, b) => (a.offset <= b.offset ? a : b))
    : null;
  return {
    facing,
    months,
    closest,
    monthsClear: months.filter(m => m.band === 'none').map(m => m.month),
    lowDeg: LOW_SUN_DEG,
  };
}
