  ...
  _all = fs.readdirSync(dir)
    ...
    .map(file => {
      ...
      return {
        ...
        /* Same field name a pipeline article uses, so one provenance panel
           serves both. A note usually needs none — its figures come from
           shortcodes over this site's own filed data, which lib/provenance.js
           credits separately — but a note ABOUT something published elsewhere
           should name it. */
        ...
        /* `draft: true` takes a note off the site without mangling it.
           A pipeline article has a status a person can change from /studio; a
           file note had nothing — the only ways to unpublish one were to blank
           its title, which loses the title, or to post-date it, which lies
           about when it was written. Deleting the file was the third option
           and the worst. */
        draft: data.draft === true || !date || date > today
