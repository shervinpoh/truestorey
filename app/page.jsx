...
import { catalogue, hdbIndex, allUrls, allTowns, projects, getIndex, storey } from '../lib/data/query.js';
 ...
 */
/* The map's frame, read once. The points are fetched by the client from
   /api/island; the page only needs the box, so its aspect ratio is reserved
   before any script runs (the PriceMap lesson in CLAUDE.md). */
let _frame;
function mapFrame() {
  ...
  try {
    ...
    _frame = { bbox: m.bbox, aspect: `${((e - w) * 1000).toFixed(0)} / ${((n - s) * 1000).toFixed(0)}` };
  } catch { _frame = null; }
  return _frame;
}
    ...
    <main className="shell wide home-atlas">
      <section className="hero">
        ...
        <div className="heromap">
          ...
          {mapFrame() && <IslandDots bbox={mapFrame().bbox} aspect={mapFrame().aspect} />}
        </div>
      </section>
