...
publishes it.** Make.com is the wire between them. A separate source-brief
scenario reports the search result every morning, including an explicit quiet
day, so silence is no longer ambiguous.
...
**`scripts/make/truestorey-articles.blueprint.json` is the writer scenario.** Import it
and nine modules arrive wired, with both filters and the iterator. The request
...
### Most days should produce no article, and that is the design
...
filters exist to end the writer run early, and on a quiet day it costs one
operation. The separate brief still confirms that the search ran and found
nothing material.
...
## It is live — repaired 22 Sep 2026

The writer is scheduled daily at 07:15. Verified end to end: Perplexity returned a URA release,
...
with a `/studio` link. Every HTTP module now evaluates non-2xx/3xx states as
errors. Apps Script notification calls follow the ContentService redirect,
...
**A green run is not evidence.** All live HTTP modules now have error evaluation
enabled. There is one exception Make cannot infer: Apps Script ContentService
can return an HTML error page with status 200. That is why each Apps Script call
keeps `Parse response` off, follows all redirects, then passes the raw body to a
terminal JSON parser. After any change, inspect that final parser's bundle and
require `ok`, `notified` and `channel`; for the article itself, also query the
database:
...
It is the same lesson as `ingest:sora` in `AGENTS.md` — an exit code is a claim,
not evidence. Do not turn HTTP error evaluation or the terminal acknowledgement
parsers off.
...
Seven placeholders, all keys or URLs, none in the file:
...
| Module 8, URL, query and body | the Apps Script `/exec` URL, `WA_WEBHOOK_KEY` and `MAKE_SECRET` |

The source-brief blueprint uses the same Perplexity key and the same three Apps
Script values. Keep them as placeholders in the versioned files; fill them only
inside Make.
...
`scripts/10_Articles.gs` is integrated into the existing **Property CRM** Apps
Script project. Version 26 added authenticated `articles` and `article_brief`
routes in `07_Bot.gs`, idempotent filing by article id, slug and source URL,
and per-row notification evidence. A retry can deliver a failed notification
without creating a second article.
...
Script Properties: `MAKE_SECRET` is also held by the two Make scenarios;
`STUDIO_PASSWORD` is the same value as `.env.local` and Vercel. The webhook key
stays in the query string because Meta requires the same Apps Script deployment
to serve the verification GET.
